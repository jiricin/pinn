...
class PINN:
    def __init__(self, i_loc, i_mat, i_mdf, d_loc, d_val, n_loc, n_val, n_dir, t_loc, t_dir, t_mat):
    ...
    def closure(self):
        ...
        self.loss = eq_loss_L + d_loss_L + eq_loss_R + d_loss_R + t_loss + t_dx_loss + n_loss_L + n_loss_R
        self.loss.backward()
...
if __name__ == '__main__':
    ...
    ds_omega[1].boundary_group_add('neumann_A', 6, [0, 1], connect_ends=False, color='orange')
    ds_omega[0].boundary_group_add('neumann_A', 4, [0, 1], connect_ends=False, color='orange')
    ds_omega[1].boundary_group_add('neumann_A', 2, [0, 1], connect_ends=False, color='orange')
    ds_omega[0].boundary_group_add('neumann_A', 0, [0, 1], connect_ends=False, color='orange')
    ...
    ds_omega[1].boundary_group_add('neumann_B', 1, [2, 3], connect_ends=False, color='red')
    ds_omega[0].boundary_group_add('neumann_B', 3, [2, 3], connect_ends=False, color='red')
    ds_omega[1].boundary_group_add('neumann_B', 5, [2, 3], connect_ends=False, color='red')
    ds_omega[0].boundary_group_add('neumann_B', 7, [2, 3], connect_ends=False, color='red')
    ...
    for k in range(2):
        ...
        d_val[k] = np.append(dirichlet_const_A * np.ones_like(d_loc_A[0]), dirichlet_const_B * np.ones_like(d_loc_B[0]))

        # generate neumann samples
        n_loc_A = np.array(ds_omega[k].sample_boundary(n_count, label='neumann_A'))
        n_loc_B = np.array(ds_omega[k].sample_boundary(n_count, label='neumann_B'))

        n_loc[k] = np.append(n_loc_A, n_loc_B, axis=1)
        n_val[k] = np.append(neumann_const_A * np.ones_like(n_loc_A[0]), neumann_const_B * np.ones_like(n_loc_B[0]))
        n_dir[k] = np.append(np.array([np.zeros_like(n_loc_A[0]), np.ones_like(n_loc_B[0])]),
                             np.array([np.zeros_like(n_loc_A[0]), (-1) * np.ones_like(n_loc_B[0])]), axis=1)
    ...
    # create PINN
    pinn = PINN(i_loc, i_mat, i_mdf, d_loc, d_val, n_loc, n_val, n_dir, t_loc, t_dir, t_mat)
