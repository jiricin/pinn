...
class PINN:
    def __init__(self,
                 loc_L, loc_mat_L, loc_matdif_L, dirichlet_loc_L, dirichlet_samples_L,
                 loc_R, loc_mat_R, loc_matdif_R, dirichlet_loc_R, dirichlet_samples_R,
                 transition_loc):
        ...
        self.loc_mat_L = torch.tensor(loc_mat_L.reshape(-1, 1), dtype=torch.float32)
        self.loc_matdif_x_L = torch.tensor(loc_matdif_L[0].reshape(-1, 1), dtype=torch.float32)
        self.loc_matdif_y_L = torch.tensor(loc_matdif_L[1].reshape(-1, 1), dtype=torch.float32)
        ...
        self.loc_mat_R = torch.tensor(loc_mat_R.reshape(-1, 1), dtype=torch.float32)
        self.loc_matdif_x_R = torch.tensor(loc_matdif_R[0].reshape(-1, 1), dtype=torch.float32)
        self.loc_matdif_y_R = torch.tensor(loc_matdif_R[1].reshape(-1, 1), dtype=torch.float32)
    ...
    def evaluate_transition_dx(self, model, x, y, mat):
    ...
    def closure(self):
        ...
        equation_prediction_L = self.evaluate_equation(self.model_L, self.loc_x_L, self.loc_y_L, self.loc_mat_L, self.loc_matdif_x_L, self.loc_matdif_y_L)
        ...
        transition_dx_prediction_L = self.evaluate_transition_dx(self.model_L, self.transition_x, self.transition_y, self.loc_mat_L)
        equation_prediction_R = self.evaluate_equation(self.model_R, self.loc_x_R, self.loc_y_R, self.loc_mat_R, self.loc_matdif_x_R, self.loc_matdif_y_R)
        ...
        transition_dx_prediction_R = self.evaluate_transition_dx(self.model_R, self.transition_x, self.transition_y, self.loc_mat_R)
    ...
    def plot_equation_loss(self, title, filename):
        ...
        values_L = self.evaluate_equation(self.model_L, self.loc_x_L, self.loc_y_L, self.loc_mat_L, self.loc_matdif_x_L, self.loc_matdif_y_L).detach().numpy().reshape(-1)
        values_R = self.evaluate_equation(self.model_R, self.loc_x_R, self.loc_y_R, self.loc_mat_R, self.loc_matdif_x_R, self.loc_matdif_y_R).detach().numpy().reshape(-1)
...
if __name__ == '__main__':
    ...
    i_mat_L = i_loc_L[0]
    i_mat_R = 3 * i_loc_R[0]
    i_matdif_L = np.array([np.ones_like(i_mat_L), np.zeros_like(i_mat_L)])
    i_matdif_R = np.array([3 * np.ones_like(i_mat_R), np.zeros_like(i_mat_R)])
    ...
    d_samples_L = 1 * np.ones_like(d_loc_L[0])
    d_samples_R = 1 * np.ones_like(d_loc_R[0])
    ...
    # create PINN
    pinn = PINN(i_loc_L, i_mat_L, i_matdif_L, d_loc_L, d_samples_L,
                i_loc_R, i_mat_R, i_matdif_R, d_loc_R, d_samples_R,
                t_loc)
