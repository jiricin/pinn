...
class PINN:
    def __init__(self,
                 ...
                 transition_loc, transition_dir, material):
        ...
        self.transition_dir_x = torch.tensor(transition_dir[0].reshape(-1, 1), dtype=torch.float32, requires_grad=True)
        self.transition_dir_y = torch.tensor(transition_dir[1].reshape(-1, 1), dtype=torch.float32, requires_grad=True)
    ...
    def evaluate_transition_dx(self, model, x, y, dir_x, dir_y, material):
        ...
        return material * (dx * dir_x + dy * dir_y)
    ...
    def closure(self):
        ...
        transition_dx_prediction_L = self.evaluate_transition_dx(self.model_L, self.transition_x, self.transition_y, self.transition_dir_x, self.transition_dir_y, self.material_L)
        ...
        transition_dx_prediction_R = self.evaluate_transition_dx(self.model_R, self.transition_x, self.transition_y, self.transition_dir_x, self.transition_dir_y, self.material_R)
    ...
    def plot_trained_function_old(self, title, filename, steps=25):
    ...
    def plot_trained_function(self, title, filename):
        ...
        values_L = self.evaluate(self.model_L, self.loc_x_L, self.loc_y_L).detach().numpy().reshape(-1)
        values_R = self.evaluate(self.model_R, self.loc_x_R, self.loc_y_R).detach().numpy().reshape(-1)
...
if __name__ == '__main__':
    ...
    t_loc_count = 15
    t_boundary_x = np.append(np.array([0]), np.linspace(0, 2, t_loc_count - 1))
    t_boundary_x = np.append(t_boundary_x, np.array([2]))
    t_boundary_y = (t_boundary_x - 2) ** 2 / 4
    t_boundary_y[0] = 0
    t_boundary_y[-1] = 1

    # domain preparation
    ds_left = DomainSampler([Boundary([np.asarray(t_boundary_x)[:-1],
                                       np.asarray(t_boundary_y)[:-1]])])
    ds_right = DomainSampler([Boundary([np.asarray(t_boundary_x)[1:],
                                        np.asarray(t_boundary_y)[1:]])])

    ds_left.boundary_group_add('dirichlet', 0, [-1, 0, 1], connect_ends=False, color='blue')
    ds_left.boundary_group_add('transition', 0, list(range(1, t_loc_count)), connect_ends=False, color='green')
    ds_right.boundary_group_add('dirichlet', 0, [-2, -1, 0], connect_ends=False, color='blue')
    ...
    d_samples_R = d_loc_R[0] * d_loc_R[1]

    # generate transition samples
    t_count = 100

    t_loc = np.array(ds_left.sample_boundary(t_count, label='transition'))
    t_direction_x = (t_loc[0] - 2) / 2  # [1, (x-2)/2] -> [(x-2)/2, -1]
    t_direction_y = - (1 - (t_direction_x ** 2)) ** 0.5
    t_direction = np.array([t_direction_x, t_direction_y])
    ...
    # create PINN
    pinn = PINN(i_loc_L, d_loc_L, d_samples_L, i_loc_R, d_loc_R, d_samples_R, t_loc, t_direction, material)
